...
(async () => {
   if (process.platform !== 'win32') {
      console.log(`nothing to build on platform ${process.platform}`);
      return;
   }

   console.log(
      ncc('Green') +
         `Starting build process...${ncc()} (this process only needs to be done once)\nDon\'t worry, if the build process fails, the program would still works.`
   );
   ...
   try {
      ...
      console.log(ncc('Red') + `Error while building!${ncc()}\nError: ${e.stack}`);
      return;
   }

   console.log(ncc('Green') + `Build completed!${ncc()}`);
})();

async function buildGetIPath(srcPath, executableName) {
   console.log(ncc('Dim') + `Building ${executableName}...` + ncc());

   if (!fs.existsSync(srcPath)) {
      console.log(
         ncc('Red') +
            `"${srcPath}" not found!${ncc()}\ncan not find the cpp source file, make sure to run this script in the correct directory`
      );
      return false;
   }
