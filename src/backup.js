   ...
   return new Promise(async (resolve, reject) => {
      try {
         ...
         const backupPath = path.join(
            config.backupFolder,
            `BACKUP_${currentDateTime}@${_global.version}.zip`
         );
...
function getBackupList() {
   ...
   return fs
      .readdirSync(config.backupFolder, { withFileTypes: true })
 ...
 * @returns {Date|null}
 */
function resolveBackupTimestamp(backupFileName) {
   // BACKUP_2024-07-21T03+14+36.709Z@1.0.0.zip
   const dateStr = backupFileName
      .slice(
         // 2024-07-21T03+14+36.709Z
         backupFileName.indexOf('_') + 1,
         backupFileName.indexOf('@')
      )
