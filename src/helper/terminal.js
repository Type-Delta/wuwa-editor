...
const km = {
   ...
   isArrow: (key) =>
      key[0] == "\u001b" && key[1] == "[" && "ABCD".includes(key[2]),
};

class Events {
   #listners = new Map();
   on(eventName, callback) {
      if (!this.#listners.has(eventName)) this.#listners.set(eventName, []);
...
module.exports = class Terminal {
   ...
   constructor(suggestion = null, lineHead = null, ctrl_C_Kill = true) {
      ...
      const headLen = ex_length(this.lineHead, this.redundancyLv);

      this.allowedInputs = headLen > 0;
      this.#lHeadLength = headLen;
   ...
   prompt(msg = null, enableSuggestion = false) {
      return new Promise((resolve, reject) => {
         if (this.prompting) {
            process.emitWarning(
               "more than one prompt() is being used at the same time",
            );
            return;
         }
         ...
         if (msg) {
            this.write(msg);
            this.#linePromptLength = ex_length(msg); // #lineLength is the length of the last line
         }
         this.prompting = true;
   ...
   promptChoice(choices, options = {}) {
      ...
      function createChoices(selected) {
         ...
         for (let i = 0; i < choices.length; i++) {
            if (i == selected)
               choice += `${ncc("BgWhite") + ncc("Black")} ${choices[i]} ${ncc("Reset")}`;
   ...
   log(msg, ...optionalParam) {
      ...
      this.#content[this.height - 1] =
         msg?.toString() + " " + optionalParam?.join(" ") + "\n";
      ...
      if (replace) {
         ...
         if (this.#content[this.#cursorPos[1]]) {
            this.#content[this.#cursorPos[1]] =
               ...
               strSlice(
                  this.#content[this.#cursorPos[1]],
                  ex_length(b_line, this.redundancyLv),
               );
   ...
   on(eventName, callback) {
      const eventNameIsValid =
         ...
         eventName == "resize";
      if (!eventNameIsValid)
         throw new Error(`No such Event, given "${eventName}"`);
   ...
   reposCursor = () => {
      this.cursorTo(
         ex_length(this.#lineMsg, this.redundancyLv),
         this.height - 1,
      );
   };
   ...
   #updateScreenElements(clearFloatsOnUpdate = true) {
      ...
      for (let i = this.#floats.length; i > 0; i--) {
         ...
         const bgStyle = f.bgColor
            ? typeof f.bgColor == "number"
               ? ncc(f.bgColor, "bg")
               : ncc(f.bgColor)
            : "";
   ...
   #handlesInput(key) {
      ...
      if (!this.allowedInputs) {
         if (km.isArrow(key)) this.#handleARROW_KEYS(key);
         return;
      }
      ...
      switch (key) {
         case km.ENTER:
            this.#clearScreenElements();
            const msg = this.#lineMsg
               .slice(this.#lHeadLength + this.#linePromptLength)
               .trim();
            process.stdout.write(this.#lineMsg + "\n");
         ...
         case km.CTRL_BACKSPACE:
            return;
         default:
            if (km.isBackspace(key)) {
               if (this.#lineLength <= Math.abs(this.#lHeadLength + this.#linePromptLength))
                  return;
    ...
    */
   #getContent = () =>
      cleanString(this.#lineMsg).slice(
         this.#lHeadLength + this.#linePromptLength,
      );
   ...
   #suggest() {
      ...
      const suggestions = search(this.suggestions, writingMsg, {
         maxResult: 3,
      });
      ...
      for (let i = 0; i < suggestions.length; i++) {
         if (!suggestions[i].string.startsWith(writingMsg)) {
            if (i == suggestions.length - 1)
               //if no suitable suggestion found set #lastSuggested = null
